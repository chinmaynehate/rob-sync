...
update_pi() {
    ...
    sshpass -p "$SSH_PASSWORD" ssh -o StrictHostKeyChecking=no pi@192.168.12.1 <<EOF
        ...
        rm -rf new_pid.py || { echo "Failed to remove old file on $pi_ssid"; exit 1; }
        wget -O new_pid.py "$WGET_URL" || { echo "Failed to download new file on $pi_ssid"; exit 1; }
        nohup python3 new_pid.py $pi_suffix > /dev/null 2>&1 &
EOF
